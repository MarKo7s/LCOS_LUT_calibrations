#!/usr/bin/env python3
"""Tag and push a meadowlark_lut release from pyproject.toml version."""
...
def resolve_tag_message(version: str, message: str | None, from_changelog: bool) -> str:
    ...
    if from_changelog:
        ...
        return f"Release meadowlark_lut {version}\n\n{body}"
    return f"Release meadowlark_lut {version}"
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "(prepended with 'Release meadowlark_lut {version}')"
        ),
    )
    parser.add_argument(
        "--dry-run",
    ...
    print("Install with:")
    print(
        f'  pip install "meadowlark_lut @ git+{REPOSITORY_URL}@{tag}"'
    )
